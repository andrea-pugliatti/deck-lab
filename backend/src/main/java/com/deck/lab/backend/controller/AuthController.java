...
/**
 * REST Controller handling user authentication, session creation, registration, and token
 * management.
 *
 * <p>
 ...
 * </p>
 * <p>
 * In a Layered Architecture, this class acts as a primary adapter (driving adapter) exposing REST
 * APIs to external clients. It delegates business validation and authentication logic to the
 * service layer ({@link AuthService}), and maps service outputs back into standard HTTP Response
 * Entities.
 * </p>
 *
 * <p>
 ...
 * </p>
 * <ul>
 * <li>{@code @RestController}: A specialized version of <i>@Controller</i> that automatically
 * configures HTTP responses. Every controller method returning an object implicitly serializes the
 * output directly to JSON in the HTTP response body (combining <i>@Controller</i> and
 * <i>@ResponseBody</i>).</li>
 * <li>{@code @RequestMapping("/api/auth")}: Establishes a base URI namespace path. All endpoints
 * defined in this class are prefixed with <i>/api/auth</i>.</li>
 * <li>{@code @PostMapping}: Binds HTTP POST requests to designated methods, conforming to REST
 * conventions for state-changing operations (like logging in, registering, or refreshing
 * tokens).</li>
 * <li>{@code @Valid}: Triggers standard Jakarta Bean Validation on incoming request objects (like
 * {@link LoginRequestDto} or {@link RegisterRequestDto}) before invoking the method body.</li>
 * </ul>
 *
 * <p>
 ...
 * </p>
 * <p>
 * To protect authenticated sessions from Cross-Site Scripting (XSS) attacks, this controller
 * implements a hybrid token delivery strategy: Short-lived JWT Access Tokens are returned in the
 * JSON response body, enabling the client application to use them in standard authorization
 * headers. Long-lived Refresh Tokens are stored inside HTTP cookies configured with
 * {@code HttpOnly}, {@code Secure}, and {@code SameSite=Lax} properties. This prevents client-side
 * JavaScript from accessing the refresh token, protecting sessions from being hijacked if malicious
...
public class AuthController {
    ...
    /**
     * Registers a new user account, returning JWT access token on success and setting the refresh
    ...
    /**
     * Rotates/refreshes the user session token. Checks rate limiting by client IP, verifies
     * expiration, and returns a new JWT access token and rotated refresh token cookie.
     *
