...
public class GetFormatRulesTool implements Function<FormatRulesRequest, FormatRulesResponse> {
    ...
    /**
     * Executes the format rules retrieval tool, querying database rules and deck sizes.
     *
    ...
    public FormatRulesResponse apply(FormatRulesRequest request) {
        if (request.format() == null || request.format()
                .isBlank()) {
        ...
        Format format;
        try {
            format = Format.fromString(request.format()
                    .trim());
        } catch (IllegalArgumentException e) {
            return new FormatRulesResponse(request.format(),
                    List.of(), "Unknown format: "
                            + request.format()
                            + ". Supported: GOAT, EDISON, TENGU_PLANT, HAT, OCG, TCG.");
        }
