...
/**
 * Service managing AI-driven deck generation and card synergy recommendations. Orchestrates calls
 * using the deep AI generation sub-modules.
 */
@Service
public class GenerationService {
    ...
    /**
     * Generates a complete, legally compliant deck list using an iterative refinement loop.
     *
     * <p>
     ...
     * <li>The draft card list is resolved against the database and format rules.</li>
     * <li>If any unresolved card names or validation warnings exist, the service enters a
     * refinement loop (up to 5 steps) and prompts the LLM again, feeding back the specific errors
     * to resolve.</li>
     * <li>The loop terminates early if the deck is fully resolved and compliant.</li>
     * </ol>
     *
     * @param request the DTO parameters specifying archetype, strategy, and format rules
     * @return the finalized deck list, matching database cards, and any validation warnings
     */
    ...
    public DeckGenerationResponseDto generateDeck(DeckGenerateRequestDto request) {
        ...
        List<ResolvedCardEntry> resolved = cardResolver.resolveCards(aiDeck != null
                ? aiDeck.getCards()
                : List.of());
        List<String> unresolvedNames = getUnresolvedNames(aiDeck != null
                ? aiDeck.getCards()
                : List.of());
        ...
        while ((!unresolvedNames.isEmpty() || !warnings.isEmpty()) && step < maxSteps) {
            step++;
            Prompt refinementPrompt = promptBuilder.buildRefinementPrompt(request,
                    resolved,
                    unresolvedNames,
                    warnings,
                    converter.getFormat());
        ...
        return new DeckGenerationResponseDto(deckName,
                description.length() > 255
                        ? description.substring(0, 255)
                        : description,
                request.getFormatName(), matchedCards, warnings);
    }

    /**
     * Identifies which card names suggested by the LLM are unrecognized by the database.
     *
     ...
     *
     * @param request  the generation parameters containing format and strategy rules
    ...
    private List<String> getValidationWarnings(DeckGenerateRequestDto request,
                                               List<ResolvedCardEntry> resolved,
                                               String deckName) {
    ...
    public List<CardSuggestionResponseDto> suggestCards(DeckSuggestRequestDto request) {
        ...
        CardSuggestionListResponseDto aiSuggestions = responseParser
                .parseSuggestionResponse(responseContent);
